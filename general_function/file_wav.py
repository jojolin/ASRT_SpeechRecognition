...
def GetFrequencyFeature3(wavsignal, fs):
    ...
    wav_arr = np.array(wavsignal)
    logd('wavsignal:', wavsignal.shape)
    wav_length = wav_arr.shape[1]
...
if(__name__=='__main__'):
    ...
    freq_feature = GetFrequencyFeature3(wave_data,fs)
    logd('freq_feature:', freq_feature.shape)
    mfcc_feature = GetMfccFeature(wave_data,fs)
    logd('mfcc_feature:', mfcc_feature.shape)
    #freimg = freq_feature.T
    #plt.subplot(111)
    #plt.imshow(freimg)
